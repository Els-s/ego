...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <meta n="format-detection" content="telephone=no" />
    <meta n="viewport" content="width=device-width, initial-scale=1.0, minimum-scale=1.0, maximum-scale=1.0, user-scalable=no" />
    <meta n="format-detection" content="telephone=no" />
...
<form id="orderForm" class="hide" action="/order/create.html" method="post">
		<input type="hidden" n="paymentType" value="1"/>
		<c:forEach items="${cartList }" var="cart" varStatus="status">
			<c:set var="totalPrice"  value="${ totalPrice + (cart.price * cart.num)}"/>
			<input type="hidden" n="orderItems[${status.index}].itemId" value="${cart.id}"/>
			<input type="hidden" n="orderItems[${status.index}].num" value="${cart.num }"/>
			<input type="hidden" n="orderItems[${status.index}].price" value="${cart.price}"/>
			<input type="hidden" n="orderItems[${status.index}].totalFee" value="${cart.price * cart.num}"/>
			<input type="hidden" n="orderItems[${status.index}].title" value="${cart.title}"/>
			<input type="hidden" n="orderItems[${status.index}].picPath" value="${cart.images[0]}"/>
		</c:forEach>
		<input type="hidden" n="payment" value="<fmt:formatNumber groupingUsed="false" maxFractionDigits="2" minFractionDigits="2" value="${totalPrice/100 }"/>"/>
		<input type="hidden" n="orderShipping.receiverName" value="老魏"/>
		<input type="hidden" n="orderShipping.receiverMobile" value="18666666666"/>
		<input type="hidden" n="orderShipping.receiverState" value="广东"/>
		<input type="hidden" n="orderShipping.receiverCity" value="广州"/>
		<input type="hidden" n="orderShipping.receiverDistrict" value="天河区"/>
		<input type="hidden" n="orderShipping.receiverAddress" value="元岗横路 慧通产业广场"/>
</form>
